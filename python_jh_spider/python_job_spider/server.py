...
def xueli():
    #打开数据库连接
    conn = pymysql.connect(host='localhost', user='root', password='123456', port=3306, db='lagou',
                           charset='utf8mb4')
    #创建一个游标对象cursor
    cursor = conn.cursor()
    #执行sql语句
...
def data():
    ...
    page = (page-1)*limit
    conn = pymysql.connect(host='localhost', user='root', password='123456', port=3306, db='lagou',charset='utf8mb4')
    cursor = conn.cursor()
